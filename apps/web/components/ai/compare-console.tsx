        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_260px]">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold text-tv-ink">
              Chọn 2-4 điểm đến và xem khác biệt khi lên kế hoạch thật
            </h2>
          </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {destinations.slice(0, 18).map((destination) => (
            <button
              ...
              className={
                selected.includes(destination.slug)
                  ? 'rounded-full bg-tv-blue px-3 py-2 text-sm font-semibold text-white'
                  : 'rounded-full border border-tv-border bg-tv-bg px-3 py-2 text-sm font-semibold text-tv-ink'
              }
            >
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.slug} className="border-b border-tv-border">
                <td className="px-4 py-4 font-bold text-tv-ink">
                  {
                    getDestinationCopy(
                      destinations.find((item) => item.slug === row.slug) ?? destinations[0]!,
                    ).name
                  }
                </td>
                ...
                <td className="px-4 py-4 font-semibold text-tv-orange">
                  {comparisonVerdictLabel(row.verdict)}
                </td>
              </tr>
            ))}
          </tbody>

  ...
  return (
    <div className="rounded-2xl border border-[#d9ecfb] bg-[#f7fbff] p-4">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-black text-[#0277d4]">Khách {index + 1}</p>
        {canRemove && (
          <button
            type="button"
            onClick={onRemove}
            className="rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs font-black text-red-600 hover:bg-red-100"
          >
            Xóa
          </button>
        )}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <label className="sm:col-span-3">
          <span className="mb-1 block text-xs font-black uppercase tracking-[0.12em] text-[#6f8594]">
            Họ và tên *
          </span>
          <input
            type="text"
            ...
            required
            className="w-full rounded-xl border border-[#c8d5e3] bg-white px-3 py-2.5 text-sm font-bold text-[#071827] outline-none focus:border-[#0277d4] focus:ring-2 focus:ring-[#0277d4]/15"
        ...
        <label>
          ...
          <select
            ...
            className="w-full rounded-xl border border-[#c8d5e3] bg-white px-3 py-2.5 text-sm font-bold text-[#071827] outline-none focus:border-[#0277d4]"
          >
          ...
          <div className="space-y-6">
            ...
            <CommerceSurface>
              <h2 className="flex items-center gap-2 text-xl font-black">
                <User size={20} className="text-[#0277d4]" aria-hidden="true" />
                Thông tin liên hệ
              </h2>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="sm:col-span-2">
                  <span className="mb-1 flex items-center gap-1.5 text-xs font-black uppercase tracking-[0.12em] text-[#6f8594]">
                  ...
                  <input
                    ...
                    required
                    className="w-full rounded-xl border border-[#c8d5e3] bg-[#f7fbff] px-4 py-3 font-bold text-[#071827] outline-none transition focus:border-[#0277d4] focus:ring-2 focus:ring-[#0277d4]/15"
                    placeholder="Nguyễn Văn A"
                  />
                </label>
                <label>
            ...
            <CommerceSurface>
              <div className="flex items-center justify-between">
                <h2 className="flex items-center gap-2 text-xl font-black">
                  <Users size={20} className="text-[#0277d4]" aria-hidden="true" />
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#d9ecfb] bg-white font-black text-[#0277d4] hover:bg-[#eef7ff] disabled:cursor-not-allowed disabled:opacity-40"
                    aria-label="Giảm số khách"
                  >
                    <Minus size={14} aria-hidden="true" />
                  </button>
                  <span className="w-6 text-center text-sm font-black">{guestCount}</span>
                  <button
                    type="button"
                    onClick={addGuest}
            ...
            <CommerceSurface>
              <h2 className="flex items-center gap-2 text-xl font-black">
                <Tag size={20} className="text-[#0277d4]" aria-hidden="true" />
                Mã giảm giá
              </h2>
              <div className="mt-4 flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-xl border border-[#c8d5e3] bg-[#f7fbff] px-4 py-3 font-bold text-[#071827] outline-none transition focus:border-[#0277d4] focus:ring-2 focus:ring-[#0277d4]/15"
                  placeholder="Nhập mã giảm giá..."
                />
                <button
                  type="button"
                  onClick={handleApplyCoupon}
                  className="rounded-xl border border-[#0277d4] bg-[#eef7ff] px-4 py-3 text-sm font-black text-[#0277d4] hover:bg-[#d9ecfb]"
                >
                  Áp dụng
                </button>
              </div>
              {couponApplied && (
                <p className="mt-2 text-sm font-black text-[#0f8b7b]">
                  ✓ Mã giảm giá sẽ được áp dụng khi xác nhận đặt tour.
                </p>
              )}
              {couponError && (
                <p className="mt-2 text-sm font-black text-red-600">{couponError}</p>
              )}
            </CommerceSurface>
          ...
          <aside className="h-fit space-y-4 lg:sticky lg:top-24">
            <CommerceSurface>
              ...
              <div className="mt-4 space-y-2">
                <p className="font-black text-[#071827]">{tour.title}</p>
              ...
              {/* Departure */}
              {selectedDeparture && (
                <div className="mt-4 flex items-center gap-2 rounded-xl bg-[#eef7ff] px-3 py-2 text-sm">
              ...
              {/* Price breakdown */}
              <div className="mt-5 space-y-2 border-t border-[#edf4fa] pt-4 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-[#476273]">
                    {formatVnd(displayPrice)} × {guestCount} khách
                  </span>
                  <span className="font-black text-[#071827]">{formatVnd(subtotal)}</span>
                </div>
                {couponApplied && couponCode && (
                  <div className="flex items-center justify-between">
                    <span className="text-[#0f8b7b]">Mã: {couponCode}</span>
                    <span className="font-black text-[#0f8b7b]">Áp dụng khi xác nhận</span>
                  </div>
                )}
                <div className="flex items-center justify-between border-t border-[#edf4fa] pt-2">
                  ...
                  <span className="text-xl font-black text-[#ff5f12]">{formatVnd(total)}</span>
                </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#ff6d1a] px-4 py-4 font-black text-white shadow-[0_12px_26px_rgba(255,109,26,0.2)] transition hover:bg-[#e95c0a] disabled:cursor-not-allowed disabled:opacity-60"
              >
                {submitting ? (
                  <>
            ...
            <Link
              ...
              className="block text-center text-sm font-bold text-[#0277d4] hover:underline"
            >
              ← Quay lại trang tour
            </Link>
          </aside>

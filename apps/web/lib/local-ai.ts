...
export async function getStructuredChatAnswer(
  message: string,
  contextSlug?: string,
): Promise<AiChatStructuredAnswer> {
...
function normalizeFastApiAnswer(
  ...
  return {
    ...
    provider: {
      runtime: 'local',
      chatProvider: text(provider.chat_provider, fallback.provider.chatProvider) as
        | 'ollama'
        | 'sample',
      model: text(provider.model, fallback.provider.model),
      embeddingProvider: text(provider.embedding_provider, fallback.provider.embeddingProvider) as
        | 'ollama'
        | 'sample',
    ...
    realtimeWarning:
      raw.realtime_warning === null
        ? undefined
        : text(raw.realtime_warning, fallback.realtimeWarning ?? ''),
  };
}

function normalizeTrip(
  raw: Record<string, unknown>,
  fallback: AiChatStructuredAnswer['itinerary'],
) {
  return {
    ...fallback,
    destination: text(raw.destination, fallback.destination),

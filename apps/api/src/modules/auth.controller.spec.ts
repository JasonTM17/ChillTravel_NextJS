...
describe("AuthController", () => {
  ...
  describe("refresh", () => {
    ...
    it("propagates UnauthorizedException for invalid refresh token", async () => {
      ...
      await expect(
        controller.refresh({ refreshToken: "invalid-token" } as any)
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe("getMe", () => {
    ...
    it("propagates UnauthorizedException when user not found", async () => {
      vi.mocked(service.getMe).mockRejectedValue(
  ...
  describe("updateMe", () => {
    it("calls authService.updateMe with userId and dto, returns updated profile", async () => {
      ...
      const updatedProfile = { ...MOCK_USER_PROFILE, fullName: "Updated Name", phone: "0901234567" };

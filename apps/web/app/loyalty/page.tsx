        ...
        <section className="space-y-5">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-4 text-3xl font-bold">
                  {currentTier.points.toLocaleString('vi-VN')} điểm demo
                </h2>
                <p className="mt-2 text-sm font-bold text-white/82">
                  Cần thêm {currentTier.nextTierPoints - currentTier.points} điểm mẫu để lên hạng
                  tiếp theo.
                </p>
              </div>
              <Link
                href="/booking/demo"
                className="inline-flex items-center justify-center gap-2 rounded-tv bg-tv-orange px-5 py-3 font-bold text-white"
              >
                Dùng ưu đãi demo
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <CommerceMetric
              label="Voucher mẫu"
              value="3"
              helper="Không có giá trị thanh toán thật."
              tone="orange"
            />
            <CommerceMetric
              label="Gói offline"
              value="5"
              helper="Lưu itinerary, checklist và QR demo."
              tone="teal"
            />
            <CommerceMetric
              label="Đặt chỗ demo"
              value={String(userBookingSummaries.length)}
              helper="Dùng cho dashboard tài khoản."
            />
          </div>
              ...
              {loyaltyTiers.map((tier) => (
                <article key={tier.id} className="rounded-tv border border-tv-border bg-tv-bg p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <StatusPill tone={tier.id === currentTier.id ? 'teal' : 'blue'}>
                  ...
                  <div className="mt-4 rounded-tv bg-white p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {tier.demoRewards.map((reward) => (
                        <StatusPill key={reward} tone="orange">
                          {reward}
                        </StatusPill>
                      ))}
                    </div>
          ...
          <CommerceSurface>
            ...
            <div className="mt-5 space-y-3">
              {userBookingSummaries.map((booking) => (
                <div
                  key={booking.id}
                  className="grid gap-3 rounded-tv border border-tv-border bg-tv-bg p-4 md:grid-cols-[150px_minmax(0,1fr)_160px] md:items-center"
                >
                  ...
                  <p className="text-right font-bold text-tv-orange">
        ...
        <aside className="h-fit space-y-4 lg:sticky lg:top-24">
          ...
          <CommerceSurface>
            ...
            <BoundaryList
              items={[
                demoPaymentWarning,
                'Điểm, voucher và nâng hạng chỉ là dữ liệu mẫu.',
                'Không có hoàn tiền, thu phí hoặc billing thật.',
              ]}
            />
          </CommerceSurface>
          <CommerceSurface>
            ...
            <div className="mt-4 grid gap-3">
              {(
                ...
                <Link
                  key={String(label)}
                  href={href}
                  className="flex items-center gap-3 rounded-tv bg-tv-bg p-4 font-bold text-tv-ink hover:text-tv-blue"
                >
          ...
          <CommerceSurface>
            <div className="flex gap-3">
              ...
              <p className="text-sm font-bold leading-6 text-tv-ink-3">
                Dashboard này dùng để thể hiện logic sản phẩm, không đại diện cho chương trình khách
                hàng thật.
              </p>
            </div>
          </CommerceSurface>

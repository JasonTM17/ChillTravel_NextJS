...
export function findDestination(query: string): Destination | undefined {
  ...
  return destinations.find((destination) => {
    ...
    return name.includes(normalized) || normalized.includes(name) || normalized.includes(destination.slug);
  });
}

const personalityProfiles: Record<TravelStyle, { keywords: string[]; description: string; traits: string[] }> = {
  "Food Hunter": {
    keywords: ["food", "street", "eat", "coffee", "local", "an", "am thuc", "cafe", "market"],
    description: "You plan trips around markets, signature dishes, cafes, and local cooking rituals.",
    traits: ["Street food curiosity", "Neighborhood-first routes", "Flexible mealtimes"]
  },
  "Culture Seeker": {
    keywords: ["culture", "history", "museum", "temple", "heritage", "van hoa", "lich su", "old town"],
    description: "You care about stories, etiquette, architecture, museums, and the deeper rhythm of a place.",
    traits: ["Context-rich days", "Local etiquette", "Slow discovery"]
  },
  "Beach Lover": {
    keywords: ["beach", "sea", "island", "sunset", "bien", "dao", "snorkel"],
    description: "You want open water, sunset pacing, seafood, and soft recovery time between activities.",
    traits: ["Coastal stays", "Sunset windows", "Light packing"]
  },
  "Mountain Adventurer": {
    keywords: ["mountain", "trek", "hike", "viewpoint", "nui", "adventure", "loop"],
    description: "You look for elevation, scenic roads, crisp mornings, and routes that feel earned.",
    traits: ["Early starts", "Weather checks", "Route discipline"]
  },
  "Luxury Escaper": {
    keywords: ["luxury", "resort", "spa", "private", "premium", "boutique", "fine"],
    description: "You prefer fewer moves, stronger stays, private transfers, and curated experiences.",
  ...
  "Family Planner": {
    ...
    description: "You need lower-risk pacing, reliable stays, nearby food, and offline-ready plans.",
    traits: ["Safety-first", "Offline checklist", "Shorter transfers"]
  },
  "World Wanderer": {
    keywords: ["world", "bucket", "global", "city", "multi", "international", "the gioi"],
    description: "You like big contrasts, cross-border inspiration, and globally recognizable highlights.",
...
export function detectTravelStyle(input: string | TravelQuizAnswer[]): TravelPersonalityResult {
  ...
  const ranked = Object.entries(personalityProfiles)
    .map(([style, profile]) => {
      const keywordScore = profile.keywords.reduce((score, keyword) => score + (normalized.includes(keyword) ? 18 : 0), 0);
      const destinationAffinity = destinations.filter((destination) => {
        const haystack = normalizeTravelText([...destination.tags, ...destination.travelStyles, destination.summary].join(" "));
  ...
  const winner = ranked[0]?.score ? ranked[0] : { style: "Culture Seeker" as TravelStyle, score: 64 };
  ...
  return {
    ...
    recommendedDestinationSlugs: recommendedDestinationSlugs.length ? recommendedDestinationSlugs : destinations.slice(0, 4).map((destination) => destination.slug)
  };
}
...
export function simulateBudget(input: BudgetSimulationInput): BudgetSimulationResult {
  ...
  const breakdown = {
    ...
    food: Math.round(dailyBase * 0.22 * foodMultipliers[input.foodLevel] * input.days * input.travelers),
    transport: Math.round(dailyBase * 0.16 * transportMultipliers[input.transportLevel] * input.days),
    activities: Math.round(dailyBase * 0.2 * activityMultipliers[input.activityLevel] * input.days * input.travelers)
  };
  ...
  return {
    ...
    breakdown,
    adjustmentNotes: [
      input.hotelLevel === "luxury" ? "Prioritize fewer hotel moves and private transfers." : "Hotel level keeps the trip flexible.",
      input.foodLevel === "street" ? "Street food routes increase local texture and lower cost." : "Food budget supports curated meals and cafe stops.",
      input.activityLevel === "packed" ? "Packed pacing needs stronger rest windows and transport buffers." : "Pacing leaves room for weather and local discoveries."
    ],
    itineraryBias: input.activityLevel === "slow" ? "chill" : input.activityLevel === "packed" ? "packed" : "balanced"
  };
}

export function compareDestinations(slugs: string[], style: TravelStyle = "Culture Seeker"): DestinationComparison[] {
  return slugs.slice(0, 4).map((slug) => {
    ...
    const familyFit = haystack.includes("family") ? 94 : destination.safetyLevel === "high" ? 84 : 70;
    ...
    const safetyFit = destination.safetyLevel === "high" ? 92 : destination.safetyLevel === "medium" ? 78 : 58;
    const budgetFit = destination.budgetMax < 5000000 ? 90 : destination.budgetMin < 2500000 ? 78 : 62;
    const aiScore = Math.round((foodFit + familyFit + nightlifeFit + activityFit + safetyFit + budgetFit) / 6);
    return {
...
export function moodSearch(query: string): MoodSearchResult {
  ...
  const budget = normalized.includes("luxury") || normalized.includes("resort") ? "luxury" : normalized.includes("cheap") || normalized.includes("tiet kiem") ? "budget" : "mid-range";
  const pace = normalized.includes("packed") || normalized.includes("nhieu") ? "packed" : normalized.includes("chill") || normalized.includes("yen binh") ? "chill" : "balanced";
  const style = detectTravelStyle(query).style;
  const result = destinations
    .filter((destination) => {
      const haystack = normalizeTravelText([...destination.tags, ...destination.travelStyles, destination.summary, destination.longDescription].join(" "));
...
export function localAiAnswer(query: string): AiAnswer {
  ...
  const normalized = normalizeTravelText(query);
  const realTimePattern = /(real-time|realtime|current|today|flight price|visa|weather now|gia ve bay|thoi tiet hien tai)/i;
  ...
  return {
    ...
    answer: limited
      ...
      : "I recommend " + destination.name + " for your " + personality.style + " style. Start with food, culture, and a balanced pace, then adjust hotel and transport choices with the Smart Budget Simulator.",
    citations: [
      { title: "ChillTravel knowledge: " + destination.name, sourceId: "destinations/" + destination.slug + ".md", chunkId: destination.slug + "-overview" }
    ],
    itineraryDraft: itinerary,
    toolCalls: [
      { name: "detect_travel_style", status: "ok", summary: "Detected " + personality.style + " from the user query." },

    ...
    <PageShell eyebrow="Vé máy bay mẫu" title="Tìm chuyến bay demo rõ giá, dễ so sánh và không dùng dữ liệu real-time">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px]">
        <section className="space-y-5">
          <CommerceSurface className="bg-[#1f9be0] text-white">
            <form action="/flights" className="grid gap-3 rounded-[22px] bg-white p-3 text-[#071827] lg:grid-cols-[1fr_1fr_1fr_1fr_132px]">
              ...
              <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#ff6d1a] px-4 py-3 text-sm font-black text-white">
          ...
          <CommerceSurface>
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.14em] text-[#0277d4]">Kết quả chuyến bay</p>
                <h2 className="mt-2 text-2xl font-black">3 lựa chọn demo cho tuyến phổ biến</h2>
              </div>
          ...
          <div className="space-y-4">
            {flightOffers.map((offer) => (
              <article key={offer.id} className="grid overflow-hidden rounded-2xl border border-[#d9ecfb] bg-white shadow-[0_14px_34px_rgba(2,68,120,0.08)] md:grid-cols-[minmax(0,1fr)_220px]">
                <div className="p-5">
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-black text-[#476273]">{offer.duration}</p>
                      <div className="my-2 h-px w-full min-w-24 bg-[#b8d8f0]" />
                      <p className="text-xs font-bold text-[#6f8594]">{offer.stops}</p>
                    </div>
                    <FlightTime city={offer.to} time={offer.arriveTime} align="right" />
                  </div>
                  <div className="mt-5 flex flex-wrap gap-2 text-sm font-bold text-[#476273]">
                    <span className="inline-flex items-center gap-2 rounded-full bg-[#f7fbff] px-3 py-1.5">
                      <BriefcaseBusiness size={16} className="text-[#0277d4]" aria-hidden="true" />
                      {offer.baggage}
                    </span>
                ...
                <aside className="flex flex-col justify-between border-t border-[#edf4fa] bg-[#fbfdff] p-5 md:border-l md:border-t-0">
                  <div>
                    ...
                    <p className="mt-1 text-2xl font-black text-[#ff5f12]">{formatVnd(offer.price)}</p>
                    <p className="mt-1 text-xs font-bold text-[#6f8594]">mỗi khách, mock fare</p>
                  </div>
                  <Link href="/booking/demo" className="mt-5 inline-flex items-center justify-center gap-2 rounded-xl bg-[#ff6d1a] px-4 py-3 text-sm font-black text-white">
                    Chọn chuyến demo
        ...
        <aside className="h-fit space-y-4 lg:sticky lg:top-24">
          <TrustBanner compact />
          <CommerceSurface>
            <h2 className="text-xl font-black">Cam kết dữ liệu</h2>
...
function FlightTime({ city, time, align = "left" }: { city: string; time: string; align?: "left" | "right" }) {
  return (
    <div className={align === "right" ? "text-left md:text-right" : ""}>
      <p className="flex items-center gap-2 text-2xl font-black md:block">
        <Clock3 className="text-[#0277d4] md:hidden" size={18} aria-hidden="true" />
        {time}
      </p>
      <p className="mt-1 text-sm font-bold text-[#476273]">{city}</p>
    </div>
  );
}

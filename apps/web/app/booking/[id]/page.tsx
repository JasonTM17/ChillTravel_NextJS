...
export default async function BookingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const destination =
    destinations.find((item) => item.slug === id) ??
    destinations.find((item) => item.slug === 'da-nang') ??
    destinations[0]!;
  const nights = 4;
    ...
    <div className="rounded-tv border border-[#f0b3ad] bg-[#ffe4e1] p-4 text-[#9f1239] shadow-[0_10px_28px_rgba(159,18,57,0.08)]">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-[#9f1239]/78">
            Mọi nhà cung cấp trong trang này là local/mô phỏng/thử nghiệm. Không phát sinh giao dịch
            thật, không lưu số thẻ, không vượt rào luật thanh toán.
          </p>
        </div>
      </div>
    </div>
    ...
    <ol className="mt-7 grid gap-3 md:grid-cols-4" aria-label="Tiến trình đặt chỗ demo">
      {checkoutSteps.map((step, index) => {
        ...
        return (
          <li
            key={step}
            className="flex items-center gap-3 rounded-tv border border-tv-border bg-white p-3"
          >
            <span
              className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-bold ${active ? 'bg-tv-blue text-white ring-4 ring-[#b8ddff]' : done ? 'bg-tv-blue text-white' : 'bg-tv-border text-[#8b99a7]'}`}
            >
              {done ? <CheckCircle2 size={18} aria-hidden="true" /> : index + 1}
            </span>
            <span
              className={`text-sm font-bold ${active ? 'text-tv-blue' : done ? 'text-tv-ink' : 'text-[#8b99a7]'}`}
            >
              {step}
            </span>
          </li>
  ...
  return (
    <aside className="overflow-hidden rounded-tv border border-tv-border bg-white shadow-tv-card">
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(180deg, rgba(7,24,39,0.02), rgba(7,24,39,0.24)), url(${getDestinationImage(destination.slug)})`,
        }}
      />
      <div className="p-5">
        <p className="inline-flex rounded-full bg-tv-blue-light px-3 py-1 text-xs font-bold text-tv-blue">
          Gói lưu trú demo
        </p>
        ...
        <div className="mt-5 space-y-3 text-sm font-bold text-tv-ink-3">
          <DetailRow
            icon={CalendarDays}
            label={`${formatDateVi(new Date('2026-08-12'))} - ${formatDateVi(new Date('2026-08-16'))} (${nights} đêm)`}
          />
        ...
        <div className="mt-5 rounded-tv bg-tv-blue-light p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-tv-ink-3">
            Hủy miễn phí trước 20:00 ngày 10/08/2026. Sau thời điểm này, hệ thống chỉ mô phỏng phí
            hủy 50%.
          </p>
        </div>
      </div>
    ...
    <section className="space-y-5">
      ...
      <div className="rounded-tv border border-tv-border bg-white p-5 shadow-tv-card">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          ...
          <span className="rounded-full bg-[#e7f8f5] px-3 py-1 text-xs font-bold text-[#0f8b7b]">
        ...
        <div className="mt-5 grid gap-3">
          {paymentMethods.map(([name, description, Icon, active]) => (
            <button
              key={name}
              type="button"
              className={`flex items-center gap-3 rounded-tv border p-4 text-left transition ${active ? 'border-tv-blue bg-tv-blue-light ring-2 ring-tv-blue/10' : 'border-tv-border bg-white hover:border-tv-blue'}`}
            >
              <span
                className={`grid h-5 w-5 shrink-0 place-items-center rounded-full border ${active ? 'border-tv-blue bg-tv-blue' : 'border-[#8b99a7]'}`}
              >
...
function FormField({
  ...
  return (
    <label className={wide ? 'md:col-span-2' : ''}>
      ...
      <input
        defaultValue={value}
        className="w-full rounded-tv border border-[#c8d5e3] bg-tv-bg px-4 py-3 font-bold text-tv-ink outline-none transition focus:border-tv-blue focus:ring-2 focus:ring-tv-blue/15"
      />
    </label>
  );
}
  ...
  return (
    <aside className="h-fit space-y-5 lg:sticky lg:top-24">
      <div className="rounded-tv border border-tv-border bg-white p-5 shadow-tv-card">
        ...
        <Link
          href={`/booking/${destination.slug}?confirmed=demo`}
          className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-tv bg-tv-orange px-4 py-4 font-bold text-white shadow-[0_14px_28px_rgba(255,109,26,0.22)] transition hover:bg-tv-orange-dark"
        >
      ...
      <div className="rounded-tv border border-tv-border bg-tv-blue-light p-5 text-center">
        ...
        <p className="mt-2 text-sm leading-6 text-tv-ink-3">
          Vé điện tử demo sẽ được tạo sau khi xác nhận. Mã đặt chỗ mẫu: CT-QR-
...
function SummaryRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span className={`text-right font-bold ${positive ? 'text-[#0f8b7b]' : 'text-tv-ink'}`}>
        {value}
      </span>
    </div>
  );

...
describe('api http hardening', () => {
  ...
  it('logs in demo users and enforces admin RBAC', async () => {
    ...
    const userToken = sign({ sub: 'user-id', email: 'user@wanderviet.com', role: 'USER' }, secret, {
      expiresIn: '15m',
    });
    const adminToken = sign(
      { sub: 'admin-id', email: 'admin@wanderviet.com', role: 'ADMIN' },
      secret,
      { expiresIn: '15m' },
    );
  ...
  it('booking endpoints require authentication', async () => {
    ...
    const noToken = await api('POST', '/bookings', {
      body: {
        tourId: 'any',
        contactName: 'Test',
  ...
  it('serves local AI endpoints without an OpenAI key', async () => {
    ...
    const mood = await api('POST', '/ai/mood-search', {
      body: { query: 'quiet beaches and seafood' },
    });
    ...
    expect(
      chat.body.data.quickActions.some((action: { id: string }) => action.id === 'estimate_budget'),
    ).toBe(true);

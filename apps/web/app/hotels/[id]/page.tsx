...
function generateGalleryImages(destinationSlug: string, count: number): string[] {
...
function LocationMap({ address, hotelName }: { address: string; hotelName: string }) {
  return (
    <CommerceSurface>
...
function GuestReviews({ reviews }: { reviews: MockReview[] }) {
  ...
  return (
    <main className="min-h-screen bg-sky-surface/30 text-ink">
      ...
      <section className="mx-auto max-w-[1180px] px-4 py-6">
        ...
        {/* Two-column layout: Content + Sticky Price Panel */}
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px]">
          ...
          {/* Right Column: Sticky Price Summary (desktop only) */}
          <StickyPriceSummary
            rooms={hotel.rooms}
            selectedRoomId={selectedRoomId}
            hotelSlug={hotel.slug}
            ...
            {selectedRoomId ? (
              <>
                ...
                <p className="text-lg font-bold text-orange-cta">
                  {formatVnd(hotel.rooms.find((r) => r.id === selectedRoomId)?.nightlyPrice ?? 0)}
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-lg font-bold text-orange-cta">
                  {formatVnd(Math.min(...hotel.rooms.map((r) => r.nightlyPrice)))}
                </p>
              </>
            )}
          </div>

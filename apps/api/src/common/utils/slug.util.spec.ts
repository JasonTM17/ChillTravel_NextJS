...
describe("generateSlug", () => {
  ...
  it("collapses whitespace and punctuation", () => {
    expect(generateSlug("Northern Vietnam Adventure")).toBe(
...
describe("ensureUniqueSlug", () => {
  ...
  it("throws when all attempts up to maxAttempts are taken", async () => {
    ...
    await expect(
      ensureUniqueSlug("ha-noi", exists, 3)
    ).rejects.toThrow(/exhausted 3 attempts/);
  });

  it("throws on empty base slug", async () => {
    await expect(
      ensureUniqueSlug("", async () => false)
    ).rejects.toThrow(/baseSlug must be non-empty/);
  });
});

...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => ({}))) as { message?: string; contextSlug?: string };
  const message = typeof body.message === "string" && body.message.trim() ? body.message : "Đà Nẵng đi 3 ngày ăn gì?";

...
export function LocalChatConcierge({ initialAnswer }: { initialAnswer: AiChatStructuredAnswer }) {
  ...
  const latestAnswer = useMemo(
    () => [...turns].reverse().find((turn) => turn.answer)?.answer ?? initialAnswer,
    [turns, initialAnswer],
  );
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="rounded-tv border border-tv-border bg-white p-4 shadow-tv-card md:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-tv-border pb-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-tv-blue">
              Trợ lý local-first
            </p>
        ...
        <div className="mt-5 space-y-4">
          {turns.map((turn) => (
            ...
            >
              ...
              <div
                className={`max-w-[92%] rounded-tv p-4 md:max-w-[78%] ${turn.role === 'user' ? 'bg-tv-blue text-white' : 'bg-tv-bg text-tv-ink'}`}
              >
        ...
        <form
          onSubmit={onSubmit}
          className="mt-5 flex flex-col gap-3 rounded-tv border border-tv-border bg-tv-bg p-3 md:flex-row"
        >
          <input
            value={input}
          ...
          <button
            disabled={isPending}
            className="inline-flex items-center justify-center gap-2 rounded-tv-sm bg-tv-orange px-5 py-3 font-bold text-white disabled:opacity-70"
          >
...
function Avatar({ icon }: { icon: 'assistant' | 'user' }) {
  return (
    <div
      className={`grid h-10 w-10 shrink-0 place-items-center rounded-tv ${icon === 'assistant' ? 'bg-tv-blue-light text-tv-blue' : 'bg-[#fff3e8] text-[#b45309]'}`}
    >
      {icon === 'assistant' ? (
        ...
        <div className="mt-3 space-y-2">
          {answer.itinerary.days.slice(0, 3).map((day) => (
            <div key={day.day} className="rounded-tv-sm bg-tv-bg p-3">
              ...
              <p className="mt-1 text-xs font-bold leading-5 text-tv-ink-3">
                {[...day.morning, ...day.afternoon, ...day.evening].slice(0, 3).join(' · ')}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-tv-sm bg-white p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {answer.clarifyingQuestions.map((question) => (
              <StatusPill key={question.id} tone="orange">
                {question.question}
              </StatusPill>
            ))}
          </div>
        ...
        {answer.quickActions.map((action) =>
          action.href ? (
            <Link
              key={action.id}
              href={action.href}
              className="inline-flex items-center justify-between rounded-tv-sm bg-tv-bg px-4 py-3 text-sm font-bold text-tv-ink transition hover:text-tv-blue"
            >
              {action.label}
            ...
            <button
              key={action.id}
              type="button"
              className="inline-flex items-center justify-between rounded-tv-sm bg-tv-bg px-4 py-3 text-left text-sm font-bold text-tv-ink"
            >
              {action.label}
      ...
      <div className="mt-4 space-y-3">
        {answer.citations.map((citation) => (
          <div
            ...
            className="rounded-tv-sm bg-tv-bg p-3"
          >
            <p className="font-bold">{citation.sourceId}</p>
            <p className="mt-1 text-xs font-bold text-tv-ink-3">
              Chunk: {citation.chunkId} · {citation.trustTier ?? 'sample'}
            </p>
          </div>
        ))}
      </div>
      <p className="mt-4 rounded-tv-sm bg-[#fff3e8] p-3 text-xs font-bold leading-5 text-[#b45309]">
        Trợ lý local không bịa vé bay, visa hoặc thời tiết hiện tại. Khi cần dữ liệu live, hãy kiểm
        tra nguồn chính thức.
      </p>
    </section>
  );
}

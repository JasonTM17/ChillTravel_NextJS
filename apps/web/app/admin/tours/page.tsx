      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.title} onChange={(e) => setStr("title", e.target.value)} required />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">Điểm đến *</label>
            <select className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.destinationId} onChange={(e) => setStr("destinationId", e.target.value)} required>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min={1} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.durationDays} onChange={(e) => setNum("durationDays", e.target.value)} required />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Số đêm</label>
              <input type="number" min={0} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.durationNights} onChange={(e) => setNum("durationNights", e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Giá gốc (VND) *</label>
              <input type="number" min={0} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.basePrice} onChange={(e) => setNum("basePrice", e.target.value)} required />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Giá khuyến mãi (VND)</label>
              <input type="number" min={0} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.salePrice ?? ""} onChange={(e) => setForm((p) => ({ ...p, salePrice: e.target.value ? Number(e.target.value) : undefined }))} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Tối đa khách *</label>
              <input type="number" min={1} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.maxGuests} onChange={(e) => setNum("maxGuests", e.target.value)} required />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Tối thiểu khách</label>
              <input type="number" min={1} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.minGuests} onChange={(e) => setNum("minGuests", e.target.value)} />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Chỗ còn lại *</label>
              <input type="number" min={0} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.availableSlots} onChange={(e) => setNum("availableSlots", e.target.value)} required />
            </div>
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">URL ảnh đại diện</label>
            <input className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.imageUrl ?? ""} onChange={(e) => setStr("imageUrl", e.target.value)} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">Mô tả ngắn</label>
            <input className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.shortDescription ?? ""} onChange={(e) => setStr("shortDescription", e.target.value)} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">Mô tả đầy đủ *</label>
            <textarea className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" rows={3} value={form.description} onChange={(e) => setStr("description", e.target.value)} required />
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#EAF7FF] p-6">
      ...
      <nav className="mb-4 text-sm text-gray-500">
        ...
        <span className="font-semibold text-[#0277D4]">Tours</span>
      </nav>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      {item.featured ? <span className="text-[#FF6D1A] font-bold">★</span> : <span className="text-gray-300">—</span>}
                    </td>

...
const questions = [
  {
    ...
    options: [
      'buổi sáng văn hóa đi chậm',
      'nhiều điểm ngắm cảnh và hoạt động',
      'nhịp an toàn cho gia đình',
    ],
  },
  {
    id: 'anchor',
    ...
    options: [
      'ẩm thực đường phố chợ quán cà phê',
      'biển đảo hoàng hôn',
      'bảo tàng đền chùa di sản',
    ],
  },
  {
    id: 'comfort',
    label: 'Mức thoải mái',
    options: [
      'tiết kiệm phương tiện công cộng hostel',
      'boutique thoải mái địa phương',
      'resort cao cấp xe riêng spa',
    ],
  },
];
...
export function PersonalityQuiz() {
  ...
  const recommended = destinations
    .filter((destination) => result.recommendedDestinationSlugs.includes(destination.slug))
    .slice(0, 4);
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[420px_1fr]">
      <div className="rounded-tv border border-tv-border bg-white p-5 shadow-tv-card">
        ...
        <p className="mt-3 text-tv-ink-3">
          {travelStyleDescription(result.style, result.description)}
        </p>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {result.traits.map((trait) => (
            <span
              key={trait}
              className="rounded-full bg-tv-blue-light px-3 py-1 text-sm font-semibold text-tv-blue"
            >
              {traitLabel(trait)}
            </span>
          ))}
      ...
      <div className="space-y-5">
        {questions.map((question) => (
          <fieldset
            key={question.id}
            className="rounded-tv border border-tv-border bg-white p-5 shadow-tv-card"
          >
            <legend className="font-bold text-tv-ink">{question.label}</legend>
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {question.options.map((option) => {
                const selected =
                  answers.find((answer) => answer.id === question.id)?.value === option;
                return (
                  <button
                    key={option}
                    type="button"
                    ...
                    className={
                      selected
                        ? 'rounded-tv-sm bg-tv-blue px-4 py-3 text-left font-semibold text-white'
                        : 'rounded-tv-sm border border-tv-border bg-tv-bg px-4 py-3 text-left font-semibold text-tv-ink'
                    }
                  >
                    {option}
                  </button>

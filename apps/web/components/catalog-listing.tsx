...
export function CatalogListing({ kind }: { kind: 'hotel' | 'experience' }) {
  ...
  const helper =
    kind === 'hotel'
      ? 'Giá phòng là dữ liệu mẫu local, có ghi chú hủy demo.'
      : 'Tour, vé và trải nghiệm đều dùng QR và đặt chỗ mô phỏng.';
      ...
      <aside className="h-fit space-y-4 lg:sticky lg:top-24">
        ...
        <CommerceSurface>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {['Hủy demo miễn phí', 'Gần trung tâm', 'Gia đình', 'Ẩm thực', 'Gói offline'].map(
              (filter) => (
...
function HotelCatalogRow({ hotel }: { hotel: HotelProperty }) {
  ...
  const lowestRoom = hotel.rooms.reduce<RoomOffer | undefined>(
    (lowest, room) => (!lowest || room.nightlyPrice < lowest.nightlyPrice ? room : lowest),
    undefined,
  );
  ...
  return (
    <article className="grid overflow-hidden rounded-tv border border-[tv-border] bg-white shadow-tv-card md:grid-cols-[210px_minmax(0,1fr)_210px]">
      <Link
        ...
        aria-label={`Xem chi tiết ${hotel.name}`}
      />
      <div className="p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-[tv-ink-3]">
          ...
          {hotel.amenities.slice(0, 2).map((amenity) => (
            <span
              key={amenity}
              className="inline-flex items-center gap-1 rounded-full bg-[#f3f9ff] px-3 py-1"
            >
              {amenity}
            </span>
          ))}
...
function CatalogRow({
  ...
  const primaryLabel =
    kind === 'hotel'
      ? (hotel?.name ?? `${copy.name} Comfort Stay`)
      : (destination.experiences[0] ?? `${copy.name} food walk`);
  const price =
    kind === 'hotel'
      ? (hotel?.nightlyPrice ?? destination.budgetMin)
      : Math.max(250000, Math.round(destination.budgetMin * 0.45));

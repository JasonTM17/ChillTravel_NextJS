      ...
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input
              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm font-mono uppercase focus:border-[#0277D4] focus:outline-none"
              value={form.code}
          ...
          <div>
            ...
            <input className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.description ?? ""} onChange={(e) => setStr("description", e.target.value)} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min={0} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.minOrderAmount ?? ""} onChange={(e) => setNum("minOrderAmount", e.target.value)} />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Giảm tối đa (VND)</label>
              <input type="number" min={0} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.maxDiscountAmount ?? ""} onChange={(e) => setNum("maxDiscountAmount", e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Giới hạn sử dụng</label>
              <input type="number" min={1} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.usageLimit ?? ""} onChange={(e) => setNum("usageLimit", e.target.value)} placeholder="Không giới hạn" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-gray-600">Ngày bắt đầu *</label>
              <input type="date" className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.validFrom} onChange={(e) => setStr("validFrom", e.target.value)} required />
            </div>
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">Ngày hết hạn</label>
            <input type="date" className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#0277D4] focus:outline-none" value={form.validUntil ?? ""} onChange={(e) => setForm((p) => ({ ...p, validUntil: e.target.value || undefined }))} />
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#EAF7FF] p-6">
      ...
      <nav className="mb-4 text-sm text-gray-500">
        ...
        <span className="font-semibold text-[#0277D4]">Mã giảm giá</span>
      </nav>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b last:border-0 hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono font-bold text-[#0277D4]">{item.code}</td>
                    <td className="px-4 py-3 text-gray-600">
                      {item.discountType === "PERCENT" ? "Phần trăm" : "Cố định"}
                    </td>
                    <td className="px-4 py-3 font-semibold text-[#FF6D1A]">
